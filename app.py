        ...
        if st.button(f'🚀 Generar PDF ({total_etiquetas_finales} etiquetas)'):
            ...
            for i, zpl_code in enumerate(labels):
                ...
                if pdf_content:
                    ...
                    # FIX: recortar al tamaño real de la etiqueta.
                    # Labelary devuelve hoja A4/Letter con la etiqueta arriba-izquierda,
                    # por eso se veía "corrida a la izquierda".
                    w_pt = ancho * 72
                    h_pt = alto * 72
                    page_h = float(pagina_maestra.mediabox.height)
                    llx, lly = 0, page_h - h_pt
                    urx, ury = w_pt, page_h
